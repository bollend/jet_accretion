...
def opacity_both(nu, T, n_HI, n_e, n_l, delta_gridpoints, Blu, rv, velocity_gradient, points, line='halpha'):
    ...
    C_osc          = ( np.pi * consts.e.esu.value**2 / (consts.m_e.cgs.value * consts.c.cgs.value ) )
    indices_sob = np.where( 0.1*delta_gridpoints  > (( 2 * constants.k * T / constants.m_p)**.5 / velocity_gradient) )
    indices_classic = np.where( 0.1*delta_gridpoints < (( 2 * constants.k * T / constants.m_p)**.5 / velocity_gradient) )
    ...
    abs_coeff_cgs[indices_sob_inters] = C_osc * f_line * constants.c / nu * n_l_cgs[indices_sob_inters] * (2. * delta_nu[indices_sob_inters])**-1 * velocity_gradient[indices_sob_inters]**-1 * ( 1 - np.exp(-constants.h * nu_0 / (constants.k * T )))
    # abs_coeff_cgs[indices] = C_osc * f_line * constants.c / nu * n_l_cgs[indices]     * (2. * delta_nu[indices])**-1     * velocity_gradient[indices]**-1 * ( 1 - np.exp(-constants.h * nu_0 / (constants.k * T )))
    abs_coeff_si   = abs_coeff_cgs * 1e2
    return abs_coeff_si
