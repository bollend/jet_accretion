...
created_interpol_normalisation = False
...
for ph in phases:
    ...
    for spec in spectra_observed[ph].keys():
        if created_interpol_normalisation==True:
            spectra_background_I[ph][spec] = spectra_background[ph][spec] * scaling_interpolation(spectra_wavelengths)
            spectra_observed_I[ph][spec]   = spectra_observed[ph][spec] * scaling_interpolation(spectra_wavelengths)
            ...
            created_interpol_normalisation = True
...
jet_temperature         = 5200      # The jet temperature (K)
jet_density_max         = 5.e15      # The jet number density at its outer edge (m^-3)
...
========================
"""
ratio_radius_background_over_primary     = 4
ratio_gridpoints_background_over_primary = 2
gridpoints_background                    = ratio_gridpoints_background_over_primary*gridpoints_primary


backgroundGrid = Star.Star(ratio_radius_background_over_primary*primary_radius_au, np.array([0+0.005,0.005-100*np.sin(inclination),-100*np.cos(inclination)]),
                          inclination, gridpoints_background)
backgroundGrid._set_grid()
backgroundGrid._set_grid_location()
line_of_sight = np.array([0, np.sin(inclination), np.cos(inclination)])

"""
===========================
Circumbinary disk inner rim
Gorlova et al. 2015: inner disk rim of IRAS 19135+3937 is 5-10AU
===========================
"""
# innerDiskRim = Star.Star(6, )
...
for phase in phases:
    for spectrum in spectra_observed[phase].keys():
        ...
        for (pointAGB, coordAGB) in enumerate(postAGB.grid_location):
            ...
            if jet.gridpoints is not None:
                ...
                for pointLOS in range(gridpoints_LOS-1):
                    ...
                    indices_frequencies = np.where(diff_nu < 2.5 * jet_delta_nu_thermal[pointLOS+1])
        ...
        for (pointBack, coordBack) in enumerate(backgroundGrid.grid_location):

            ### For each ray of light from a gridpoint on the post-AGB star, we calculate
            ### the absorption by the jet.
            jet._set_gridpoints(coordBack, gridpoints_LOS)
            intersection = line_plane_intersection(line_of_sight, coordBack, line_of_sight, postAGB.centre)
            distance_intersection_postAGB = np.abs(np.linalg.norm(postAGB.centre - intersection))
            jet_temperature = 5200
            distance_from_centre = np.abs(np.linalg.norm(coordBack - backgroundGrid.centre))
            if distance_from_centre > 1.:
                jet_temperature = 4200
                if distance_from_centre > 2:
                    jet_temperature = 3200
                    if distance_from_centre > 3:
                        jet_temperature = 2200
            # plt.plot(backgroundGrid.grid_location[:,1], backgroundGrid.grid_location[:,2])
            # plt.plot(postAGB.grid_location[:,1], postAGB.grid_location[:,2])
            # plt.scatter(intersection[1], intersection[2])
            # print(distance_intersection_postAGB, postAGB.get_radius())
            # plt.show()
            ...
            elif distance_intersection_postAGB < postAGB.get_radius():
            ...
            intensity += gridpoints_background**-1 * ratio_radius_background_over_primary**2 * np.array(intensity_point)
